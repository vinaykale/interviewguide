...
var JobLevelService = (function () {
    function JobLevelService(http) {
        this.http = http;
        this.headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        this.joblevelUrl = 'api/joblevels'; // URL to web api
    }
    JobLevelService.prototype.getJobLevels = function () {
        return this.http.get(this.joblevelUrl)
            .toPromise()
            .then(function (response) { return response.json().data; })
            .catch(this.handleError);
    };
    JobLevelService.prototype.getJobLevel = function (id) {
        var url = this.joblevelUrl + "/" + id;
        return this.http.get(url)
            .toPromise()
            .then(function (response) { return response.json().data; })
            .catch(this.handleError);
    };
    JobLevelService.prototype.update = function (hero) {
        var url = this.joblevelUrl + "/" + hero.id;
        return this.http
